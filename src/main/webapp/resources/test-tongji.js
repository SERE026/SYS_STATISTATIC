var jfTj = function() {
	...
	c = {
		...
		dm: ["test.yinchengmall.com"],
		js: "tongji.yinchengmall.com:9095/resources/",
		etrk: [],
		icon: '',
		ctrk: true,
		align: 1,
	...
	(mt.p.ya = function() {
		...
		(function() {
			...
			else if (document.addEventListener) document.addEventListener("DOMContentLoaded", k, r),
			window.addEventListener("load", a, r);
	...
	h.o = {
		La: "http://tongji.yinchengmall.com:9095/resources/welcome/ico",
		L: "tongji.yinchengmall.com:9095/visit/send",
		Z: "yinchengmall.com",
		ta: "hmmd",
		ua: "hmpl",
		sa: "hmkw",
		...
		function a(a, d) {
			...
			b.d(d, "Function") && (k.readyState ? k.onreadystatechange = function() {
				if ("loaded" === k.readyState || "complete" === k.readyState) k.onreadystatechange = q,
				d()
			}: k.onload = function() {
				d()
			});
		...
		l = {
			X: function() {
				c.ctrk && (b.e(document, "mouseup", l.ba()), b.e(window, "unload",
				function() {
					l.B()
		...
		function d(a) {
			if (! ("focus" == a.type || "blur" == a.type) || !(a.target && a.target != window)) m = "focus" == a.type || "focusin" == a.type ? p: r,
			b()
		}
		var k = mt.event,
		l = h.s,
		...
		d = {
			wa: function(d) {
				if ((void 0 === window._dxt || a.d(window._dxt, "Array")) && "undefined" !== typeof h.b) {
					var l = h.b.H();
					f([b.protocol, "//datax.yinchengmall.com/x.js?si=", c.id, "&dm=", encodeURIComponent(l)].join(""), d)
				}
			},
			Fa: function(b) {
		...
		e = {
			...
			ga: function() {
				...
				if (a && a.length) for (var b = 0; b < a.length; b++) {
					...
					case "_setAutoPageview":
						if (1 < d.length && (d = d[1], r === d || p === d)) e.c |= 2,
			...
			ha: function() {
				if ("undefined" === typeof window._bdhm_account || window._bdhm_account === c.id) {
					...
					d = a.length; b < d; b++) f.d(a[b], "Array") && "_trackEvent" !== a[b][0] && "_trackRTEvent" !== a[b][0] ? e.K(a[b]) : e.q.push(a[b]);
					window._hmt = e.R
				}
			},
			ia: function() {
				if (0 < e.q.length) for (var a = 0,
				b = e.q.length; a < b; a++) e.K(e.q[a]);
				e.q = q
			},
			K: function(a) {
				if (f.d(a, "Array")) {
			...
			_trackRTEvent: function(b) {
				...
				if (f.d(b, "Object")) {
					...
					else for (var l = Math.ceil(l / 900), t = "block|" + Math.round(Math.random() * k.A).toString(16) + "|" + l + "|", s = [], u = 0; u < l; u++) s.push(u),
					s.push(b.substring(900 * u, 900 * u + 900)),
					m.call(this, t + s.join("|")),
					s = []
				}
			},
		...
		a.prototype = {
			...
			Da: function() {
				var a, b, d, f, e;
				m.m = this.getData("YCM_Hm_lpvt_" + c.id) || 0;
				13 == m.m.length && (m.m = Math.round(m.m / 1E3));
				b = this.pa();
				a = 4 != b ? 1 : 0;
				if (d = this.getData("YCM_Hm_lvt_" + c.id)) {
					f = d.split(",");
				...
				f = "",
				e = 1;
				this.setData("YCM_Hm_lvt_" + c.id, d, c.age);
				this.setData("YCM_Hm_lpvt_" + c.id, m.j);
				d = m.j == this.getData("YCM_Hm_lpvt_" + c.id) ? "1": "0";
			...
			Ea: function() {
				...
				this.a.fl = d.qa();
				this.a.v = "1.0.94";
				this.a.cv = decodeURIComponent(this.getData("YCM_Hm_cv_" + c.id) || "");
			...
			Y: function(a) {
				var b = g.get("YCM_Hm_unsent_" + c.id) || "",
				d = this.a.u ? "": "&u=" + encodeURIComponent(document.location.href),
				b = encodeURIComponent(a.replace(/^https?:\/\//, "") + d) + (b ? "," + b: "");
				g.set("YCM_Hm_unsent_" + c.id, b)
			},
			M: function(a) {
				var b = g.get("YCM_Hm_unsent_" + c.id) || "";
			...
			Ca: function() {
				var a = this,
				b = g.get("YCM_Hm_unsent_" + c.id);
				if (b) for (var b = b.split(","), d = function(b) {
					f.log(m.protocol + "//" + decodeURIComponent(b).replace(/^https?:\/\//, ""),
					function(b) {
						a.M(b)
					})
				},
		...
		try {
			if (window.performance && performance.timing && "undefined" !== typeof h.b) {
				...
				var e = function() {
					...
					debugger;
					console.log("---f---",f);
					if ("tongji.yinchengmall.com" === (a.match(/^(http[s]?:\/\/)?([^\/]+)(.*)/) || [])[2]) g = f.i(a, "qid"),
					b = f.i(a, "click_t");
					console.log("----g--- ",g);
					a = g;
					e.qid = a != q ? a: "";
			...
			init: function() {
				try {
					if ("http:" === a.protocol) {
						...
						b.setAttribute("src", "http://tongji.yinchengmall.com");
		...
		if (c.comm && "undefined" !== typeof h.b) {
			...
			l = {
				click: function() {
					for (var a = [], b = d(document.getElementsByTagName("a")), b = [].concat.apply(b, d(document.getElementsByTagName("area"))), b = [].concat.apply(b, d(document.getElementsByTagName("img"))), e = /openZoosUrl\(|swt/, f = /\/LR\/Chatpre\.aspx/, g = 0, l = b.length; g < l; g++) {
						...
						m = m.getAttribute("href"); (e.test(n) || f.test(m) || k.test(m)) && a.push(b[g])
					}
					return a
				}
			},
			...
			g,
			m = "/zoosnet" + (/\/$/.test("/zoosnet") ? "": "/"),
			v = function(b, d) {
				if (g === d) return e(m + b, d),
				r;
				if (a.d(d, "Array") || a.d(d, "NodeList")) for (var f = 0,
				k = d.length; f < k; f++) if (g === d[f]) return e(m + b + "/" + (f + 1), d[f]),
				r
			};
			b.e(document, "click",
			function(b) {
